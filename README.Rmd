...
`{openFDA}` makes querying the [openFDA API](https://open.fda.gov/apis/) from R
a breeze. The API itself serves publicly available data from the FDA about 
foods, drugs, devices, and more. This data includes data such as recall 
enforcement reports, adverse events, manufacturer details, and more. Note that 
the data on openFDA has not been validated for clinical or production use.
...
There is also [FDAopenR](https://github.com/ck2136/FDAopenR/), which I couldn't quiet wrap my head around. The package appears to be in working order, though!