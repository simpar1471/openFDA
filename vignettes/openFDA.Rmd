...
The API limits results sets to 1000, and pages through results if a user submits a query with >1000 results.
By default, `openFDA()` will ask you if you want to perform paging, but this tends to break vignettes.
Therefore, we set the option `openFDA.paging` to `"yes"` in order to automatically perform paging when needed.

```{r set-paging, echo = FALSE}
...
We can suppress this warning by setting `paging` to `"no-quiet"`.
This ensures you don't page the whole dataset (which would take eleven hours) and suppresses the warning:
...
```{r example_5a_single_search_term_unrefined, out.lines = 15}
...
```{r example_5b_single_search_term_refined, out.lines = 15}
...
```{r example_5d_format_scalar_search_term, out.lines = 15}
...
```{r example_5e_format_nonscalar_search_term, out.lines = 15}
search <- openFDA(search = c("openfda.generic_name" = "amoxicillin",
                             "openfda.route" = "oral"),
                  endpoint = "drug-drugsfda",
                  limit = 1,
...
```{r example5f_format_nonscalar_search_term_with_and, out.lines = 15}
